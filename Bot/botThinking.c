...
void chooseHighestProbability(int* row, int* col,PLAYER* currentPlayer)
{
    int maxVal =-1;
    for (int i = 0; i < GRID_SIZE; i++) {
        for (int j = 0; j < GRID_SIZE; j++) {
            if (currentPlayer->probGrid[i][j] > maxVal) {
                maxVal = currentPlayer->probGrid[i][j];
                *row = i;
                *col = j;
            }
        }
...
void fireMoveBot(PLAYER *currentPlayer, PLAYER *opposingPlayer, INPUT *input)
{
    ...
    int RadarSweep = radarSweep(currentPlayer,opposingPlayer,&input);
    if (!findNextTarget(currentPlayer, &row, &col))
    {
        // If no valid target found, randomly pick
        chooseHighestProbability(&row, &col,currentPlayer);
    }
    if (RadarSweep == 1)
    {
...
void torpedoBot(PLAYER* currentPlayer, PLAYER* opposingPlayer) {
    ...
    if (!findTorpedoTarget(currentPlayer, &row, &col, &orientation)) {
        chooseHighestProbability(&row, &col,currentPlayer);  // Ensure this method works as intended
    }
